    ...
    private Optional<String> getPolicyArn(String policyName, Region awsRegion) {
        ...
        try {
            ...
            if (e.getMessage().contains("not authorized to perform")) {
                outStream.println("No permissions to lookup managed policy, looking for user defined policy...");
            }
        }
        ...
        try {
            ...
            if (e.getMessage().contains("not authorized to perform")) {
                outStream.println("No permissions to lookup IAM policy, will attempt creating one. If you wish to use"
                        + " an existing policy instead, please make sure the credentials used for setup have "
                        + "iam::getPolicy permissions for the policy resource and retry...");
            }
        }
