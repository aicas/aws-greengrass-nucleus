...
public class DefaultDockerClient {
     ...
     * @throws ImagePullServiceException           error in authenticating with the registry
     * @throws UserNotAuthorizedForDockerException when current user is not authorized to use docker
     * @throws IOException                         unexpected error
     */
    boolean login(Registry registry)
            throws ImagePullServiceException, UserNotAuthorizedForDockerException, IOException {
        ...
        if (response.exit.isPresent()) {
            if (response.exit.get() == 0) {
                ...
                throw new ImagePullServiceException(
     ...
     * @throws ImagePullRetryableException         an error that can be potentially fixed through retries
     * @throws InvalidImageOrAccessDeniedException an error indicating incorrect image specification or auth issues with
     *                                             the registry
     * @throws UserNotAuthorizedForDockerException when current user is not authorized to use docker
     * @throws IOException                         unexpected error
     */
    boolean pullImage(Image image) throws ImagePullRetryableException, InvalidImageOrAccessDeniedException,
        ...
        if (response.exit.isPresent()) {
            if (response.exit.get() == 0) {
                ...
                if (response.getOut().contains("Service Unavailable")) {
                    ...
                    throw new ImagePullRetryableException(
                            String.format("Error logging into the registry using credentials - %s", response.out));
                }
